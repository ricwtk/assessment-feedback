<template>
  <div class="w-full max-w-5xl flex flex-col gap-2 m-2">
    ...
    <Panel v-if="studentlist.length > 0" pt:content:class="flex flex-col gap-2">
      ...
      <div class="flex flex-col gap-2">
        <div>Email title</div>
        <InputText class="grow" v-model="emailtitle"/>
      </div>
      <div class="flex flex-col gap-2">
...
async function getStudentList(courseKey, pin) {
  loginerror.value = ""
  try {
    const response = await fetch(`${API_BASE_URL}/course/${courseKey}/pin`, {
      method: "POST",
      headers: {
    ...
    const data = await response.json();
    studentlist.value = Object.keys(data).sort().map(k => {
      let x = data[k]
      x.id = k
      return x
    })
...
const adminlogin = (login) => {
  selectedCourse.value = login.course
